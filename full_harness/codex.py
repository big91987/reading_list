...
REVIEW_SCHEMA = {
    ...
    "properties": {
        "status": {
            "type": "string",
            "enum": ["passed", "changes", "needs_input", "blocked"],
        },
        "summary": {"type": "string"},
        "question": {"type": "string"},
        "return_stage": {
            "type": "string",
            "enum": ["requirements", "design", "development"],
        },
...
def runtime_home(home):
    ...
    auth = (
        Path(os.environ.get("CODEX_AUTH_HOME", str(Path.home() / ".codex")))
        / "auth.json"
    )
    ...
    if not target.exists():
        if not auth.is_file():
            raise RuntimeError(
                "Runner requires a pre-authenticated Codex auth.json; no login is performed by a task"
            )
...
def invoke(
    ...
    if (workspace / ".codex").exists():
        raise ValueError(
            "This initial runtime requires project .codex configuration to be reviewed and removed from the execution workspace before enabling the managed hook"
        )
    ...
    if not isinstance(result[array_key], list) or not all(
        isinstance(x, str) for x in result[array_key]
    ):
        raise ValueError("Invalid result array")
    if review and result["return_stage"] not in [
        "requirements",
        "design",
        "development",
    ]:
