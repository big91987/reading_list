...
def catalog(home, workspace):
    ...
    process = subprocess.Popen(['codex', 'app-server', '--strict-config', '--stdio'],
            ...
            def request(number, method, params):
                ...
                while time.monotonic() < deadline:
                    ...
                    if reply.get('id') != number:
                        continue
                    if 'error' in reply:
                        raise RuntimeError('Codex native skill discovery failed: ' + str(reply['error']))
                    return reply['result']
                raise RuntimeError('Codex native skill discovery timed out')
            request(1, 'initialize', {'clientInfo': {'name': 'harness_skill_scope', 'version': '1'}})
...
def configure(home, workspace, source, allowed, config, evidence):
    ...
    expected = {str(relative_file(root, name + '/SKILL.md').resolve()) for name in allowed}
    ...
    if not expected <= paths:
        raise RuntimeError('Configured skills were not discovered by Codex: ' + str(expected - paths))
    for path in sorted(paths):
        config += '\n[[skills.config]]\npath=' + json.dumps(path) + '\nenabled=' + str(path in expected).lower() + '\n'
    (home / 'config.toml').write_text(config)
