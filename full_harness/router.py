...
SCHEMA={'type':'object','additionalProperties':False,'required':['status','summary','question','decisions'],'properties':{
 ...
 'properties':{'stage':{'type':'string','enum':STAGES[:4]},'action':{'type':'string','enum':['run','reuse','not_applicable']},
...
def validate(result,workspace,state):
    ...
    if not isinstance(items,list) or [x.get('stage') for x in items]!=STAGES[:4]:raise ValueError('Routing must assess every authoring stage once, in order')
    for item in items:
        ...
        if item['stage']=='implementation' and item['action']=='reuse' and not any(x!='issue' for x in refs):raise ValueError('Existing-code entry requires actual project files')
...
def classify(source,workspace,session,state,evidence):
    ...
    prompt=(spec.get('instruction','')+'\n'+
      ...
      '需求基线可以直接是足够明确的 Issue（evidence 使用 issue）；设计、规划对小改可无需单独开展，但要说明理由。'
      '已有代码满足功能要求可复用 implementation，由后续 verification 执行真实检查，review 独立评审；你不能跳过这两个环节或批准交付。'
