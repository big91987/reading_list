...
SCHEMA = {
    ...
    "required": ["status", "summary", "question", "decisions"],
    "properties": {
        ...
        "decisions": {
            ...
            "items": {
                "type": "object",
                "additionalProperties": False,
                "required": ["stage", "action", "reason", "evidence"],
                "properties": {
                    "stage": {"type": "string", "enum": STAGES[:3]},
                    "action": {
                        "type": "string",
                        "enum": ["run", "reuse", "not_applicable"],
                    },
                    "reason": {"type": "string"},
                    "evidence": {
                        "type": "array",
                        "items": {
                            "type": "string",
                            "description": "Use literal issue for the current Issue, otherwise an existing repository-relative file path.",
                        },
                    },
                },
            },
...
def evidence_hash(workspace, state, name):
    if name == "issue":
        return hashlib.sha256(
            json.dumps(
                {"task": state["task"], "instruction": state.get("instruction", "")},
                sort_keys=True,
            ).encode()
        ).hexdigest()
    ...
    if not p.is_file() or p.is_symlink():
        raise ValueError(
            "Routing evidence must be an existing regular project file: " + name
        )
    return hashlib.sha256(p.read_bytes()).hexdigest()


def validate(result, workspace, state):
    if set(result) != set(SCHEMA["required"]) or result["status"] not in {
        "ready",
        "needs_input",
        "blocked",
    }:
    ...
    if not all(
        isinstance(result[k], str) for k in ["status", "summary", "question"]
    ...
    if result["status"] == "needs_input" and not result["question"].strip():
        raise ValueError("Missing routing question")
    if result["status"] != "ready":
        return result
    items = result["decisions"]
    if not isinstance(items, list) or [x.get("stage") for x in items] != STAGES[:3]:
        raise ValueError("Routing must assess every authoring stage once, in order")
    for item in items:
        if (
            item["action"] not in {"run", "reuse", "not_applicable"}
        ...
        if not isinstance(refs, list) or not all(isinstance(x, str) for x in refs):
            raise ValueError("Invalid routing evidence list")
        refs = item["evidence"] = [canonical_evidence(name, state) for name in refs]
        if item["action"] == "reuse" and not refs:
            raise ValueError("Reuse requires concrete evidence")
        if item["stage"] == "requirements" and item["action"] == "not_applicable":
            raise ValueError(
                "A task always needs a requirements baseline, possibly the Issue itself"
            )
        if (
            item["stage"] == "development"
            and item["action"] == "reuse"
            and not any(x != "issue" for x in refs)
        ):
            raise ValueError("Existing-code entry requires actual project files")
        item["evidence_hashes"] = {
            name: evidence_hash(workspace, state, name) for name in refs
        }
    return result
...
def classify(source, workspace, session, state, evidence):
    ...
    contracts = {
        ...
        for name, value in state["config"]["stages"].items()
        if name in STAGES[:3]
    }
    prompt = (
        ...
        + json.dumps(
            {
                ...
                "previous_outcomes": state.get("completed", {}),
                "previous_assessment": state.get("routing"),
            },
            ensure_ascii=False,
        )
    )
    ...
    if digest(workspace) != before:
        raise ValueError("Entry assessment changed the workspace")
